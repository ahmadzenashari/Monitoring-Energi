...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {/* Header Logo */}
      <div
        className={`flex h-20 items-center border-b border-slate-200/60 ${
      ...
      <nav className="flex-1 overflow-y-auto px-4 py-6 overflow-x-hidden">
        <p
          className={`mb-4 px-3 text-[11px] font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap transition-all duration-300 ${
            ...
            return (
              <button
                ...
                className={`group flex items-center rounded-xl py-3 text-sm font-medium transition-all duration-200 ${
                  isOpen
                    ? "w-full px-3 gap-3"
                    : "w-12 px-0 justify-center mx-auto gap-0"
                } ${
                  active
      ...
      <div className="border-t border-slate-200/60 p-4">
        ...
        >
          <svg
            className="h-5 w-5 shrink-0 fill-none stroke-current transition-transform duration-200 group-hover:-translate-x-1"
            viewBox="0 0 24 24"
            strokeWidth="2"
          >
            <path
